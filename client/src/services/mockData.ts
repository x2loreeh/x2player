import type { Album, Track, Playlist } from '@shared/schema';
...
export const mockAlbums: Album[] = [
  ...
  {
    id: '3',
    name: 'dont tap the glass',
    artist: 'xqvc',
    coverArt: mockCovers[2],
    year: 2024,
    genre: 'Electronic',
    trackCount: 6,
    duration: 1200,
...
export const mockTracks: { [albumId: string]: Track[] } = {
  '1': [
    {
      id: '1-1',
      title: 'F1r3 Y0ur M4n4Ger',
      artist: 'YOUNG CARTI',
      album: 'WHOLE LOTTA MUSIC',
      albumId: '1',
      duration: 225,
      track: 1,
      year: 2024,
    ...
    {
      id: '1-4',
      title: 'Medium Length Song Title Test',
      artist: 'YOUNG CARTI',
      album: 'WHOLE LOTTA MUSIC',
      albumId: '1',
      duration: 165,
    ...
    {
      id: '1-6',
      title: 'This Is A Very Long Song Title That Should Definitely Cause Layout Problems If Not Handled Properly',
      artist: 'YOUNG CARTI',
      album: 'WHOLE LOTTA MUSIC',
      albumId: '1',
      duration: 245,
...
export class MockNavidromeService {
  ...
  async getAlbums(type: 'newest' | 'recent' | 'frequent' | 'random' = 'newest', size = 20): Promise<Album[]> {
    ...
    switch (type) {
      ...
      case 'newest':
        ...
        break;
      case 'random':
        // For random, start from a different offset to avoid duplicates with newest
  ...
  async getPlaylistTracks(playlistId: string): Promise<Track[]> {
    await new Promise(resolve => setTimeout(resolve, 300));
    // Return some sample tracks for any playlist
    return Object.values(mockTracks).flat().slice(0, 5);
  }

  getStreamUrl(trackId: string): string {
    return '#'; // No actual streaming in mock mode
  }
